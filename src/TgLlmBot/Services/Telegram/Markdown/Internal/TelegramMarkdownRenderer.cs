...
{
    private static readonly FrozenSet<char> SpecialChars = new HashSet<char>
    {
        '_',
        '*',
        '[',
        ...
        foreach (var listItemBlock in list)
        {
            ...
            if (!list.IsOrdered && item.Count > 0 && item[0] is ParagraphBlock firstPara && firstPara.Inline != null)
            {
                ...
                if (firstInline != null && firstInline.GetType().Name == "TaskList")
                {
                    ...
                    var isChecked = checkedProperty != null && checkedProperty.GetValue(firstInline) is bool checkedValue && checkedValue;
            ...
            if (list.IsOrdered)
            {
                _output.Append(CultureInfo.InvariantCulture, $"{item.Order}\\. ");
            }
            else if (isTaskList)
            {
            ...
            else
            {
                ...
                foreach (var block in item)
                {
                    ...
                    else if (block is ListBlock nestedList)
                    {
                        ...
                        _output.AppendLine();
                        var nestedContent = RenderListDirectly(nestedList, "  ");
    ...
    private static string RenderListDirectly(ListBlock list, string indent)
    {
        ...
        foreach (var listItemBlock in list)
        {
            ...
            if (list.IsOrdered)
            {
                nestedOutput.Append(CultureInfo.InvariantCulture, $"{indent}{item.Order}\\. ");
            }
            else if (isTaskList)
            {
            ...
            else
            {
                ...
                foreach (var block in item)
                {
                    ...
                    else if (block is ListBlock nestedList)
                    {
                        ...
                        var nestedListContent = RenderListDirectly(nestedList, indent + "  ");
    ...
    private void RenderQuote(QuoteBlock quote)
    {
        foreach (var block in quote)
        {
            if (block is ParagraphBlock para)
            {
                ...
                var paraContent = paraRenderer._output.ToString().Trim();
                var lines = paraContent.Split(['\r', '\n'], StringSplitOptions.RemoveEmptyEntries);
            ...
            else
            {
                ...
                var blockContent = blockRenderer._output.ToString();
